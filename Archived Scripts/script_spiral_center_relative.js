...
// Spiral where the center sprite is guaranteed at screen center (0, 0, 0)
...
function animate() {
  ...
  sprites.forEach((sprite, i) => {
    // Offset relative to scroll center
    let relativeIndex = ((i - scrollOffset + total + total / 2) % total) - total / 2;

    // Horn spiral relative to center
    const y = -relativeIndex * baseSpacing;
    const hornFactor = 1.5 - 1.0 * Math.tanh(y / 4);
    const angle = relativeIndex * (Math.PI * 2) / total * spiralTurns;
    const radius = hornFactor * 2.5;

    const x = radius * Math.cos(angle) - radius * Math.cos(0); // Offset to center (0,0,0)
    const z = radius * Math.sin(angle) - radius * Math.sin(0);
